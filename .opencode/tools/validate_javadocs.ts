...
async function runBackend(projectRoot: string, input: unknown, abort: AbortSignal) {
  const child = Bun.spawn(["uv", "run", "--script", path.join(import.meta.dir, "validate_javadocs.py"), "--repo", projectRoot], {
...
export default tool({
  description: "排除未完成的逐檔結果，驗證所有差異只有 Javadoc，然後執行 Maven 編譯與專案既有的 Javadoc 檢查。",
  args: {
