...
async function runBackend(root: string, input: unknown, abort: AbortSignal) {
  const child = Bun.spawn(["uv", "run", "--script", path.join(import.meta.dir, "prepare_javadoc_workspace.py"), "--repo", root], {
...
export default tool({
  description: "以 origin 遠端預設分支最新版本建立 Javadoc 專用分支與共用 worktree，並列出 Git 追蹤的 Maven 標準正式 Java 檔案。",
  args: {
    target_path: tool.schema.string().trim().optional().describe("不填表示整個專案；填入時只處理一個專案相對 Java 路徑，可含開頭 @。"),
  },
