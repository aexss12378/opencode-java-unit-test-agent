...
2. Skill 要求確認範圍、提出具體問題或回報阻擋事項時，立即停止並等待工程師回應；不得提交部分案例。
3. Skill 產生完整且沒有阻擋事項的候選測試後，才交給 `submit_unit_tests`。

## 工具結果

- 工具回傳 `status: candidate-check-failed` 時，先依案例的規格證據與失敗內容分類：
  - 候選測試有編譯、匯入、設定或規格轉錄錯誤：修正候選測試後重新提交。
  - 有可信規格依據的斷言與實際結果不同：不得修改預期結果或重新提交；回報可能的正式原始碼缺陷、案例編號、規格證據與實際結果，然後停止。
