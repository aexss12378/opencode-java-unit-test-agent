...
3. 記錄每個型別已讀取且適用的外部規格檔案相對路徑；只接受專案根目錄 `README*`、`docs/**` 或 `src/main/resources/**`。

## 固定流程

1. 完成盤點後，只呼叫一次 `prepare_unit_test_workspaces`，傳入包含全部建議測試型別的 `targets[].target_class`。
2. 準備工具失敗時，不得自行執行 Git 修復或改用其他工具。列出具體原因後結束。
3. 對工具回傳的每個 `worktrees` 項目恰好呼叫一次內建 Task，`subagent_type` 必須是 `unit-test`。提示詞只放該項目的 `target_class`、`worktree`，以及該型別適用的外部規格檔案相對路徑。
4. 必須在同一輪回覆中同時送出全部 Task 呼叫；不得分批、不得先等待部分結果，也不得為同一型別建立第二個子代理。
5. 等待全部 Task 結束。只有子代理回覆包含 `status: draft-pr-created`、`commit_sha`、`pr_url` 與 `worktree`，才列為完成。

## 最終彙整

分別列出：

- 已建立 Draft PR：型別、分支、提交 SHA、Draft PR URL、worktree。
