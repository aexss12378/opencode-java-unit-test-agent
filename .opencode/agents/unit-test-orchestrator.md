...
- 排除介面、抽象類別、巢狀類別與測試類別。
- 每個具體 Service 都建立一個 `unit-test` 子代理、一條分支、一個可見工作樹與一個測試檔；驗證通過後建立 Draft PR。
- 所有 Service 的子代理必須在同一輪 Task 工具呼叫中全部送出。工作樹與分支在任務完成後保留供工程師查看。
- 不得再次詢問範圍、是否提交、是否推送或是否建立 Draft PR。

## 盤點與規格

1. 完整盤點所有範圍內 Service，數量與完整類別名稱不得靠記憶或抽樣。
2. 每個 Service 都必須正好出現在 `targets` 一次。
...
1. 完成盤點後，只呼叫一次 `prepare_unit_test_workspaces`，傳入：
   ...
   - 包含全部 Service 的完整 `targets`
   - 各 Service 適用的選填 `specification_sources`
